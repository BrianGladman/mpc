/* Copyright (C) 2004, 2005, 2006, 2008, 2012 Patrick Pelissier, Paul Zimmermann,
...
{
  if (DPE_UNLIKELY (DPE_MANT (y) == 0))
    dpe_set (x, z);
  else if (DPE_UNLIKELY (DPE_MANT (z) == 0))
    dpe_set (x, y);
  else if (DPE_UNLIKELY (DPE_EXP(y) > DPE_EXP(z) + DPE_BITSIZE))
