...
Assume $x = (a,b)$ and $y = (c,d)$ with $a, c \neq 0$ and $b, d = \pm 0$.
Assume the imaginary part of $z := x^y$ is zero, which happens when
either $a > 0$, or when $a < 0$ and $c$ is an integer.
The rule we follow is the following: when we replace $b$ and $d$ by tiny
values $\varepsilon_b$ and $\varepsilon_d$ of the same sign as $b$ and $d$
respectively, if we always obtain the same sign for $\Re(z)$ when
$\varepsilon_b$ and $\varepsilon_d$ vary, then we return a zero of that sign;
otherwise, if we obtain different signs for $\Re(z)$ when
$\varepsilon_b$ and $\varepsilon_d$ vary, we return a positive zero.
